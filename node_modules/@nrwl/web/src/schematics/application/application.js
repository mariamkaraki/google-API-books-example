...
function createApplicationFiles(options) {
    return schematics_1.mergeWith(schematics_1.apply(schematics_1.url(`./files/app`), [
        schematics_1.template(Object.assign(Object.assign(Object.assign({}, options), devkit_1.names(options.name)), { tmpl: '', offsetFromRoot: devkit_1.offsetFromRoot(options.appProjectRoot) })),
...
function addProject(options) {
    return workspace_1.updateWorkspaceInTree((json) => {
        ...
        architect.lint = workspace_1.generateProjectLint(core_1.normalize(options.appProjectRoot), core_1.join(core_1.normalize(options.appProjectRoot), 'tsconfig.app.json'), options.linter, [`${options.appProjectRoot}/**/*.ts`]);
...
exports.applicationGenerator = ngcli_adapter_1.wrapAngularDevkitSchematic('@nrwl/web', 'application');
