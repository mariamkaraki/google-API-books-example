        ...
        if (options.watch) {
            ...
            return rxjs_1.from(rollupOptions).pipe(operators_1.concatMap((opts) => run_rollup_1.runRollup(opts).pipe(operators_1.catchError((e) => {
                ...
                next: (result) => {
                    if (result.success) {
                        updatePackageJson(options, context, target, dependencies, packageJson);
                        context.logger.info(`Bundle complete: ${context.target.project}`);
                    }
                    else {
                        context.logger.error(`Bundle failed: ${context.target.project}`);
                    }
                },
            }))));
        }
    ...
    return outputConfigs.map((config) => {
        ...
        const rollupConfig = {
            ...
            output: {
                ...
                name: options.umdName || devkit_1.names(context.target.project).className,
            },
